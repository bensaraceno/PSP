...
pgeFont* pgeFontLoad(const char *filename, unsigned int fontSize, enum pgeFontSizeType fontSizeType, unsigned int textureSize) {
	...
	if (FT_New_Face(library, filename, 0, &face)) return NULL;

	FT_GlyphSlot slot; GlyphInfo *gp; GlyphInfo gi[256];
	int n, count, charCount, xx, yy;
	charCount = strlen(PGE_FONT_CHARSET);
	count = charCount;

	if ((textureSize < PGE_FONT_TEXTURE_MIN_SIZE) || (textureSize > PGE_FONT_TEXTURE_MAX_SIZE)) return NULL;
	pgeFont* font = (pgeFont*) malloc(sizeof(pgeFont));
	if (!font) return NULL;

	font->texSize = textureSize;
	if (fontSizeType == PGE_FONT_SIZE_PIXELS) {
		if (FT_Set_Pixel_Sizes(face, fontSize, 0)) { pgeFontUnload(font); return NULL; }
	} else {
		if (FT_Set_Char_Size(face, fontSize<<6, 0, 100, 0)) { pgeFontUnload(font); return NULL; }
	}

	slot = face->glyph;
	for (n = 0; n < count; n++) {
		if (FT_Load_Char(face, PGE_FONT_CHARSET[n], FT_LOAD_RENDER)) { pgeFontUnload(font); return NULL; }
		...
		gi[n].glyph.unused = 0;
		gp = glyphList.next;
		while ((gp != &glyphList) && (gp->glyph.height > gi[n].glyph.height)) { gp = gp->next; }
        gi[n].next = gp;
        gi[n].prev = gp->prev;
        gi[n].next->prev = gi;
        gi[n].prev->next = gi;
    ...
    int x = 0, y = 0, ynext = 0, used = 0;
    count = 0; memset(font->map, 255, 256);
    font->texture = (unsigned char*) malloc(textureSize * textureSize>>1);
    if (!font->texture) { pgeFontUnload(font); return NULL; }
    memset(font->texture, 0, textureSize * textureSize>>1);
    for (n = 0; n < charCount; n++){
		if (FT_Load_Char(face, gi[n].c, FT_LOAD_RENDER)) { pgeFontUnload(font); return NULL; }
		if ((x + gi[n].glyph.width) > textureSize) { y += ynext; x = 0; }
		if (gi[n].glyph.height > ynext) ynext = gi[n].glyph.height;
		if ((y + ynext) > textureSize) { pgeFontUnload(font); return NULL; }
		font->map[gi[n].c] = count++;
		gi[n].glyph.x = x;
		gi[n].glyph.y = y;
        for (yy = 0; yy < gi[n].glyph.height; yy++) {
			xx = 0;
			if (x & 1) { font->texture[(x + (y + yy) * textureSize)>>1] |= (slot->bitmap.buffer[yy * slot->bitmap.width] & 0xF0); xx++; }
    ...
    for (n = 0; n < 256; n++) { if (font->map[n] == 255) font->map[n] = font->map[' ']; }
    for (n = 0; n < charCount; n++) { memcpy(&font->glyph[n], &gi[n].glyph, sizeof(gi[n].glyph)); }
	FT_Done_Face(face); FT_Done_FreeType(library); sceKernelDcacheWritebackAll();
	if (!pgeFontSwizzle(font)) { pgeFontUnload(font); return NULL; }
	sceKernelDcacheWritebackAll();
	return font;
}

pgeFont* pgeFontLoadMemory(const void *buffer, unsigned int buf_size, unsigned int fontSize, enum pgeFontSizeType fontSizeType, unsigned int textureSize) {
	FT_Library library; FT_Face face;
	if (FT_Init_FreeType(&library)) return NULL;
	if (FT_New_Memory_Face(library, buffer, buf_size, 0, &face)) return NULL;
...
int pgeFontInit(void) {
	int n; for (n = 0; n < 16; n++) clut[n] = ((n * 17) << 24) | 0xffffff;
	return 1;
}
...
int pgeFontPrint(pgeFont *font, float x, float y, unsigned int color, const char *text) {
	...
	sceGuDrawArray(GU_SPRITES, GU_TEXTURE_32BITF | GU_COLOR_8888 | GU_VERTEX_32BITF | GU_TRANSFORM_2D, length << 1, 0, v);
...
int pgeFontPrintfCenter(pgeFont *font, float y, unsigned int color, const char *text, ...) {
	if (!font) return 0;
	char buffer[256];
	va_list ap;
	va_start(ap, text);
	...
	va_end(ap);
	buffer[255] = 0;
	return pgeFontPrintCenter(font, y, color, buffer);
}

int pgeFontPrintCenter(pgeFont *font, float y, unsigned int color, const char *text) {
	float x = 240 - (pgeFontMeasureText(font, text) / 2);
	return pgeFontPrint(font, x, y, color, text);
}
