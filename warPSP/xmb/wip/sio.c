 ...
 *
 * $HeadURL: svn://svn.pspdev.org/psp/trunk/psplink/psplink/sio.c $
 * $Id: sio.c 2151 2007-01-27 10:01:57Z tyranid $
 */
...
/* DO NOT NEED TO PUT CHARACTERS, JUST NEED TO GET THEM!
...
/* Put data to SIO converting any line feeds as necessary. NOT NEEDED
int sioPutText(const char *data, int len) {
	int i;
	for (i = 0; i < len; i++) {
		// If just line feed add a carriage return.
		if (data[i] == '\n') { if (((i > 0) && (data[i-1] != '\r')) || (i == 0)) sioPutchar('\r'); }
		sioPutchar(data[i]);
...
/* DO NOT NEED TO PUT CHARACTERS JUST GET THEM! THIS DOESNT LOCK THE PRX UP.
...
/* ALL THESE FUNCTIONS LOCK UP THE PRX :(
