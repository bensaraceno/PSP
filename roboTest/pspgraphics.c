...
void drawScreenRect(Color color, int x0, int y0, int w0, int h0)
{
	if (!initialized) return;
	drawLineScreen(x0, y0, (x0 + w0), y0, color);
	drawLineScreen(x0, y0, x0, (y0 + h0), color);
	drawLineScreen(x0, (y0 + h0), (x0 + w0), (y0 + h0), color);
	drawLineScreen((x0 + w0), y0, (x0 + w0), (y0 + h0), color);
}
