...
void fillScreenRect(Color color, int x0, int y0, int width, int height) {
	if (!initialized) return;
	int x, y; Color* vram = getVramDrawBuffer();
	for (y = y0; y < y0 + height; y++) {
		for (x = x0; x < x0 + width; x++) vram[y * PSP_LINE_SIZE + x] = color;
	}
}
...
void printTextScreenCenter(int y, const char* text, u32 color) {
	int x = 240 - (4 * strlen(text));
	printTextScreen(x, y, text, color);
}

void printTextImageCenter(int y, const char* text, u32 color, Image* image) {
	int x = 240 - (4 * strlen(text));
...
void drawRectScreen(Color color, int x0, int y0, int width, int height) {
	if (!initialized) return;
	int x, y; Color* vram = getVramDrawBuffer();
	for (x = x0; x <= x0 + width; x++) { vram[y0 * PSP_LINE_SIZE + x] = color; } // Top
	for (x = x0; x <= x0 + width; x++) { vram[(y0 + height) * PSP_LINE_SIZE + x] = color; } // Bottom
	for (y = y0; y <= y0 + height; y++) { vram[y * PSP_LINE_SIZE + x0] = color; } // Left
	for (y = y0; y <= y0 + height; y++) { vram[y * PSP_LINE_SIZE + x0 + width] = color; } // Right
}

void drawButtonScreen(Button btn) {
	if (!initialized) return;
	int x, y, col; Color* vram = getVramDrawBuffer();
	for (y = btn.y; y < btn.y + btn.h; y++) {
		for (x = btn.x; x < btn.x + btn.w; x++) {
			if ((btn.bordersize > 0) &&
				((x <= btn.x + btn.bordersize - 1) || (x >= btn.x + btn.w - btn.bordersize) ||
				 (y <= btn.y + btn.bordersize - 1) || (y >= btn.y + btn.h - btn.bordersize))) {
				// Draw the border
				if (btn.borderbevel) col = 0xFFFFFFFF; else col = btn.bordercolor;
			} else {
				col = btn.fillcolor;
			}
			vram[y * PSP_LINE_SIZE + x] = col;
		}
	}
	printTextScreen((btn.x + (btn.w / 2)) - (4 * strlen(btn.text)), btn.y + (btn.h / 2) - 4, btn.text, btn.textcolor);
}
